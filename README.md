...
**選擇原因：**
- ViewModel 可讓 UI 與邏輯分離
- 可集中管理聊天狀態與資料流
- 避免將商業邏輯直接寫在畫面層

### 3. Data Layer（資料層）
包含：
- **Retrofit + OkHttp**：串接 NASA APOD API
- **Room Database**：儲存本地收藏資料
- **Repository**：整合本地與遠端資料來源

**選擇原因：**
- Retrofit 適合處理 RESTful API
- Room 適合本地資料保存，符合收藏需求
- Repository 可隔離資料來源細節，讓 ViewModel 更單純

### 為什麼選擇 MVVM？
聊天式 App 具有明確的 UI 狀態變化與資料流需求。MVVM 可讓：
- UI 專注於顯示
- ViewModel 專注於狀態與邏輯
- Data Layer 專注於資料存取

這樣的分工讓專案更清楚、可維護性更高，也更適合未來擴充更多功能。
